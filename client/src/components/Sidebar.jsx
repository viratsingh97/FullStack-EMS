...
import { CalendarIcon, ChevronRightIcon, DollarSignIcon, FileTextIcon, LayoutGridIcon, LogInIcon, MenuIcon, SettingsIcon, UserIcon, XIcon } from 'lucide-react'
...
const Sidebar = () => {
        ...
        <>
            ...
            {userName && (
                <div className='mx-3 mt-4 mb-1 p-3 rounded-lg bg-white/3 border border-white/4'>
                    <div className='flex items-center gap-3'>
                        <div className='w-9 h-9 rounded-lg bg-slate-800 flex items-center justify-center ring-1 ring-white shrink-0'>
                        ...
                        <div className='min-w-0'>
                            ...
                            <p className='text-[11px] text-slate-500 truncate'>{role === "ADMIN" ? "Administartion" : "Employee"}</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className='p-3 border-t border-white/6'>
                <button onClick={handleLogout} className='flex items-center gap-3 w-full px-3 py-2.5 rounded-md text-[13px] font-medium text-slate-400 hover:text-rose-400 hover:bg-rose-500/8 transition-all duration-150'>
                    <LogInIcon className="w-[17px] h-[17px]"/>
                    <span>Log out</span>
                </button>
            </div>
        </>
    ...
    return (
        <>
            ...
            {/*Mobile overlay*/}
            {MobileOpen && <div className='lg:hidden fixed inset-0 bg-black/10 backdrop-blur-sm z-40' onClick={() => setMobileOpen(false)} />}

            {/*Sidebar desktop*/}
            <aside className='hidden lg:flex flex-col h-full w-65 bg-linear-to-b from-slate-900 via-slate-900 to-slate-950 shrink-0 border-r border-white/4'>
                {sidebarContent}
            </aside>

            {/*Sidebar Mobile */}
            <aside className={`lg:hidden fixed insert-y-0 left-0 w-72 bg-linear-to-b from-slate-900 via-slate-900 to-slate-950 text-wh z-50 flex flex-col transform transition-transform duration-300 ${MobileOpen ? "translate-x-0" : "translate-x-full"}`}>
                {sidebarContent}
            </aside>
        </>
    )
